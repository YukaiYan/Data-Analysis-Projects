%% load data for Test3: Horizontal Displacement
...
% extract the location of the bucket for test3
threshold = 220;
[xpos1, ypos1] = extractLocation(vidFrames1_3, threshold, 1, false);
[xpos2, ypos2] = extractLocation(vidFrames2_3, threshold, 2, false);
[ypos3, xpos3] = extractLocation(vidFrames3_3, threshold, 3, false);
...
for i = 1:numFrames
    ...
    if (camera == 1)
        binaryImage = imcrop(binaryImage,[260.5 143.5 171 288]);
    elseif (camera == 2)
        binaryImage = imcrop(binaryImage,[188.5 174.5 175 246]);
        if (i >= 6 && i <= 24)
            for x = 1:200
                for y = 1:100
                    binaryImage(x,y) = 0;
    ...
    elseif (camera == 3)
        binaryImage = imcrop(binaryImage,[181.5 133.5 448 252]);    
    end
    
    % Get rid of blobs smaller than 200 pixels.
